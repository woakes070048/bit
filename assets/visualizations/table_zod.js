...
import './table_zod.css';

const $ = require('jquery');

dt(window, $);

function TableZodViz(slice, payload) {
  ...
  const datatable = container.find('.dataTable').DataTable({
    ...
    'footerCallback': function(tfoot, data, start, end, display) {
      ...
      $(api.column(0).header()).html(
        '<span>' + $(api.column(0).header()).find('span').html() + '</span><hr/>Sum/Avg'
      );
      ...
      let i = 1;
      let total;
      const avgColumns = [7, 9, 12, 13];
      for (i = 1; i < api.columns().header().length; i++) {
        total = api.column(i).data().reduce(function (a, b) {
          return intVal(a) + intVal(b);
        }, 0);
        if (data.length && avgColumns.indexOf(i) !== -1) {
          // avg
          total = (total / data.length);
        }
        // if (total) $(api.column(i).footer()).html(total.toFixed(2));
        if (total) {
          $(api.column(i).header()).html(
              '<span>' + $(api.column(i).header()).find('span').html() + '</span><hr/>' + total.toFixed(2)
          );
        }
      }
    },
